...
function App() {
  ...
  const [currentLoggedInUser, setCurrentLoggedInUser] = useState<User | undefined>(undefined);
...
// Translation
  ...
  const changeLanguageAndTranslate = async (text: string, chosenLanguage: string) => {
...
// Handle Log Ins
  ...
  const handleListItemLogIn = (emailClicked: string) => {
    setLoggedIn(true);
