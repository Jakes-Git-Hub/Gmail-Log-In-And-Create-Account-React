...
export const SignInFrontPageContainer = ({ users, userData, updateUser, text, passFoundUser }) => {
    ...
    // Change Language
    const handleLanguageSelection = (language) => updateUser({ language });
    // Email or Phone
    ...
    const handleNextClick = (e) => {
        ...
        if (emailOrPhone === '') {
            ...
            const registeredUser = users.find((user) => user.email === emailOrPhone || user.phoneNumber === emailOrPhone);
            if (registeredUser) {
    ...
    return (_jsx(_Fragment, { children: _jsx(SignInFrontPageComponent, { userData: userData, handleLanguageSelection: handleLanguageSelection, isImageLoaded: isImageLoaded, errorCondition: errorCondition, emailOrPhone: emailOrPhone, onEmailOrPhoneChange: onEmailOrPhoneChange, handleForgotEmailButtonClick: handleForgotEmailButtonClick, handleGuestModeInfoButtonClick: handleGuestModeInfoButtonClick, handleCreateAccountClick: handleCreateAccountClick, handleNextClick: handleNextClick, text: text }) }));
};
